...
class HuggingFaceService
{
    ...
    private function generateDemoFallback(string $prompt): string
    {
        ...
        if (str_contains($prompt, 'JSON')) {
            ...
            // FIX: Guard against division by zero when min === max in the range
            $prixRange = $conf['prix'][1] - $conf['prix'][0];
            $capRange  = $conf['cap'][1]  - $conf['cap'][0];

            $prix = $conf['prix'][0] + ($prixRange > 0 ? ($seed % $prixRange) : 0);
            $cap  = $conf['cap'][0]  + ($capRange  > 0 ? ($seed % $capRange)  : 0);

...
class EvenementRepository extends ServiceEntityRepository
{
     ...
     * @return Evenement[]
     */
    public function findSimilarPastEvents(Evenement $event): array
    {
        $qb = $this->createQueryBuilder('e')
            ->where('e.dateFin < :now')
            ->setParameter('now', new \DateTime())
            ->andWhere('e.id != :currentId')
            ->setParameter('currentId', $event->getId() ?? 0); // Handle new events (id null)

        if ($event->getTypeArt()) {
            $qb->andWhere('e.typeArt = :typeArt')
               ->setParameter('typeArt', $event->getTypeArt());
        }
        ...
        return $qb->setMaxResults(10) // Limit to 10 most recent similar events

...
final class EvenementBackController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager, \App\Service\AiImageService $aiImageService): Response
    {
        $check = $this->checkModuleAccess($request, 'evenement');
        if ($check) return $check;
        $user = $this->getUser();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (!$evenement->getImage()) {
                $imageUrl = $aiImageService->generateForEvent($evenement->getNom(), $evenement->getDescription());
                $evenement->setImage($imageUrl);
            }

            $entityManager->persist($evenement);
            $entityManager->flush();
            $this->addFlash('success', 'Événement créé avec une image générée par IA.');
            return $this->redirectToRoute('back_evenement_index', [], Response::HTTP_SEE_OTHER);
        }

...
final class BackController extends AbstractController
{
    ...
    private function generateReclamationSummary(Reclamation $reclamation): array
    {
        ...
        // Calculate processing time if there are responses
        if (count($responses) > 0) {
            $firstResponse = $responses->first();
            if ($firstResponse && $firstResponse->getDatereponse()) {
                $interval = $reclamation->getDatecreation()->diff($firstResponse->getDatereponse());
                $days = $interval->days;
                $hours = $interval->h;

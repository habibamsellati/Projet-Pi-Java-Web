...
final class BackController extends AbstractController
{
    ...
    private const RESPONSABLE_ACCOUNTS = [
        ...
        'proposition' => ['username' => 'proposition', 'password' => 'prop', 'label' => 'Proposition'],
        'reservation' => ['username' => 'reservation', 'password' => 'reservation', 'label' => 'Reservation'],
        'suivi_livraison' => ['username' => 'suivi_livraison', 'password' => 'suivi', 'label' => 'Suivi Livraison'],
        'commande' => ['username' => 'commande', 'password' => 'commande', 'label' => 'Commande'],
    ...
    #[Route('/utilisateurs/{id}/supprimer-definitivement', name: 'back_user_delete_permanent', methods: ['POST'])]
    public function deleteUserPermanent(int $id, UserRepository $userRepository, EntityManagerInterface $em): Response
    {
        $check = $this->checkAdminAccess();
        if ($check) return $check;
        ...
        $user = $userRepository->find($id);
        if (!$user || $user->getDeletedAt() === null) {
            $this->addFlash('error', 'Utilisateur non trouve dans la corbeille');
            return $this->redirectToRoute('back_users_deleted');
        }

        if ($admin && $user->getId() === $admin->getId()) {
            $this->addFlash('error', 'Vous ne pouvez pas vous supprimer definitivement');
            return $this->redirectToRoute('back_users_deleted');
        }
        ...
        $this->addFlash('success', 'Utilisateur supprime definitivement de la base');
        return $this->redirectToRoute('back_users_deleted');
    }
    ...
    #[Route('/reclamation/{id}/summary', name: 'back_reclamation_summary', requirements: ['id' => '\d+'], methods: ['GET'])]
    public function reclamationSummary(Reclamation $reclamation): Response
    {
        $check = $this->checkAdminAccess();
        if ($check) return $check;

        // Generate summary
        $summary = $this->generateReclamationSummary($reclamation);
    ...
    #[Route('/reclamation/{id}/create-video-call', name: 'back_reclamation_create_video_call', requirements: ['id' => '\d+'], methods: ['POST'])]
    public function createVideoCall(Reclamation $reclamation, Request $request, EntityManagerInterface $entityManager, MailerInterface $mailer): Response
    {
        $check = $this->checkAdminAccess();
        if ($check) return $check;

        $token = $request->request->get('_token');
        if (!$token || !$this->isCsrfTokenValid('create_video_call' . $reclamation->getId(), $token)) {
            $this->addFlash('error', 'Token invalide.');
            return $this->redirectToRoute('back_reclamation_show', ['id' => $reclamation->getId()]);
        }
        ...
        // Store room ID instead of full URL
        $reclamation->setVideoCallLink($roomId);
        $reclamation->setVideoCallScheduledAt(new \DateTime());
        $entityManager->flush();

        // Send email to client with video call link
        $this->sendVideoCallInvitation($reclamation, $mailer);

        $this->addFlash('success', 'Lien de visioconférence créé et envoyé au client.');
        return $this->redirectToRoute('back_reclamation_show', ['id' => $reclamation->getId()]);
    }

    #[Route('/reclamation/{id}/video-call', name: 'back_reclamation_video_call', requirements: ['id' => '\d+'], methods: ['GET'])]
    public function joinVideoCallAdmin(Reclamation $reclamation): Response
    {
        $check = $this->checkAdminAccess();
        if ($check) return $check;
        ...
        /** @var User|null $user */
        $user = $this->getUser();
        $displayName = $user ? $user->getPrenom() . ' ' . $user->getNom() . ' (Admin)' : 'Administrateur';

        return $this->render('admin/video_call.html.twig', [
            'reclamation' => $reclamation,
    ...
    private function sendVideoCallInvitation(Reclamation $reclamation, MailerInterface $mailer): void
    {
        ...
        // Generate URL to the embedded video call page in the application
        $videoLink = 'http://localhost:8000/reclamation/' . $reclamation->getId() . '/video-call';
...
<head>
    <style>
        ...
        .header { background: #2d7a4f; color: white; padding: 20px; border-radius: 8px 8px 0 0; text-align: center; }
        .content { background: #f8f9fa; padding: 30px; border: 1px solid #dee2e6; }
        .video-box { background: white; padding: 20px; margin: 20px 0; border-radius: 8px; text-align: center; border: 2px solid #2d7a4f; }
        .button { display: inline-block; padding: 15px 30px; background: #2d7a4f; color: white; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px; margin: 15px 0; }
...
<body>
    <div class="container">
        <div class="header">
            <h2 style="margin: 0;">📹 Invitation à une visioconférence</h2>
        </div>
        
        <div class="content">
            <p>Bonjour {$user->getPrenom()} {$user->getNom()},</p>
            
            <p>Un administrateur souhaite discuter avec vous par visioconférence concernant votre réclamation :</p>
            
            <div class="info">
                <strong>Réclamation #{$reclamation->getId()}</strong><br>
                {$reclamation->getTitre()}
            </div>
            
            <div class="video-box">
                <h3 style="color: #2d7a4f; margin-top: 0;">Rejoindre la visioconférence</h3>
                <p>Cliquez sur le bouton ci-dessous pour rejoindre l'appel vidéo directement dans votre espace client :</p>
                <a href="{$videoLink}" class="button">🎥 Rejoindre maintenant</a>
            </div>
            
            <div class="info">
                <strong>ℹ️ Informations importantes :</strong>
                <ul style="margin: 10px 0; padding-left: 20px;">
                    <li>La visioconférence s'ouvre directement dans votre navigateur</li>
                    ...
                    <li>Vous pouvez aussi accéder à la visio depuis la page de votre réclamation</li>
                </ul>
            </div>
            
            <p>Nous sommes impatients de résoudre votre problème rapidement.</p>
            
            <p>Cordialement,<br>L'équipe AfkArt</p>
        </div>
...
HTML;
        ...
        $email = (new Email())
            ->from($mailFrom)
            ->to($user->getEmail())
            ->subject('📹 Invitation à une visioconférence - Réclamation #' . $reclamation->getId())
            ->html($emailContent);
    ...
    private function generateReclamationSummary(Reclamation $reclamation): array
    {
        $user = $reclamation->getUser();
        ...
        $summary = [
            ...
            'date_creation' => $reclamation->getDatecreation() ? $reclamation->getDatecreation()->format('d/m/Y H:i') : 'N/A',
            'utilisateur' => [
                'nom' => $user ? $user->getNom() . ' ' . $user->getPrenom() : 'Inconnu',
                'email' => $user ? $user->getEmail() : 'N/A',
                'role' => $user ? $user->getRole() : 'N/A',
            ],
            'description' => $description,
            'ai_summary' => $aiSummary,
            'nombre_reponses' => count($responses),
            'temps_traitement' => null,
        ];

        // Calculate processing time if resolved
        if (in_array($reclamation->getStatut(), ['validee', 'rejetee']) && $reclamation->getDatecreation()) {
            $now = new \DateTime();
            $interval = $reclamation->getDatecreation()->diff($now);
            if ($interval) {
                $days = $interval->days;
                $hours = $interval->h;
                $summary['temps_traitement'] = $days > 0 ? "$days jour(s) $hours heure(s)" : "$hours heure(s)";
            }
        }
        ...
        foreach ($responses as $response) {
            ...
            $summary['reponses'][] = [
                'date' => $response->getDatereponse() ? $response->getDatereponse()->format('d/m/Y H:i') : 'N/A',
                'admin' => $admin ? $admin->getNom() . ' ' . $admin->getPrenom() : 'Inconnu',
                'contenu' => $response->getContenu(),
            ];
        }

        // Generate text summary
        $textSummary = "Réclamation #{$summary['id']} - {$summary['titre']}\n\n";
        $textSummary .= "Statut: {$summary['statut']}\n";
        $textSummary .= "Date de création: {$summary['date_creation']}\n";
        $textSummary .= "Utilisateur: {$summary['utilisateur']['nom']} ({$summary['utilisateur']['email']})\n";
        $textSummary .= "Rôle: {$summary['utilisateur']['role']}\n\n";
        $textSummary .= "Résumé automatique:\n{$aiSummary}\n\n";
        $textSummary .= "Description complète:\n{$summary['description']}\n\n";
        
        if ($summary['temps_traitement']) {
            $textSummary .= "Temps de traitement: {$summary['temps_traitement']}\n\n";
        }
        
        if (count($summary['reponses']) > 0) {
            $textSummary .= "Réponses ({$summary['nombre_reponses']}):\n";
            foreach ($summary['reponses'] as $idx => $reponse) {
                $num = $idx + 1;
                $textSummary .= "\n{$num}. {$reponse['date']} - {$reponse['admin']}\n";
                $textSummary .= "   {$reponse['contenu']}\n";
            }
        } else {
            $textSummary .= "Aucune réponse pour le moment.\n";
        }
    ...
    private function generateAISummary(string $text): string
    {
        ...
        $apiSummary = $this->generateHuggingFaceSummary($text);
        if ($apiSummary !== null) {
            $normalizedApi = mb_strtolower(trim((string) preg_replace('/\s+/', ' ', $apiSummary)));
            $normalizedText = mb_strtolower(trim((string) preg_replace('/\s+/', ' ', $text)));
            if ($normalizedApi !== '' && $normalizedApi !== $normalizedText) {
                return $apiSummary;
            }
        }
        ...
        if (mb_strlen($text) < 50) {
            ...
            $short = is_array($words) ? implode(' ', array_slice($words, 0, 18)) : $text;
            if (is_array($words) && count($words) > 18) {
                $short .= '...';
            }
            return 'Le client signale: ' . $short;
        }

        // Extract key information using simple NLP techniques
        ...
        if (count($sentences) <= 2) {
            ...
            if ($secondSentence !== '') {
                $summaryText .= '. ' . $secondSentence;
            }
            if (!preg_match('/[.!?]$/', $summaryText)) {
                $summaryText .= '.';
            }

            return 'Synthese: ' . $summaryText;
        }

        // Score sentences based on keywords
        $keywords = ['problème', 'défaut', 'erreur', 'demande', 'réclamation', 'insatisfait', 
                     'remboursement', 'remplacement', 'réparation', 'urgent', 'important',
                     'problem', 'issue', 'request', 'refund', 'replacement', 'damaged', 'defective'];
        ...
        foreach ($sentences as $sentence) {
            ...
            // Prefer sentences with numbers (dates, order numbers, etc.)
        ...
        $topSentences = array_slice($scoredSentences, 0, min(3, count($scoredSentences)));
        ...
        $normalizedSummary = mb_strtolower(trim((string) preg_replace('/\s+/', ' ', $summaryText)));
        $normalizedText = mb_strtolower(trim((string) preg_replace('/\s+/', ' ', $text)));

        if ($normalizedSummary === $normalizedText || mb_strlen($summaryText) >= mb_strlen($text)) {
            $words = preg_split('/\s+/', $text);
            $short = is_array($words) ? implode(' ', array_slice($words, 0, 20)) : $text;
            if (is_array($words) && count($words) > 20) {
                $short .= '...';
            }

            return 'Synthese: ' . $short;
        }

        return $summaryText;
    }

    private function generateHuggingFaceSummary(string $text): ?string
    {
        ...
        $prompt = sprintf(
            "Resume cette reclamation client en francais en 2 phrases maximum. " .
            "Conserve seulement le probleme principal et la demande du client.\n\nReclamation:\n%s",
            $text
        );
        ...
        try {
            ...
            if (is_array($payload) && isset($payload[0]['generated_text'])) {
                $generatedText = (string) $payload[0]['generated_text'];
            } elseif (is_array($payload) && isset($payload['generated_text'])) {
                $generatedText = (string) $payload['generated_text'];
            }
            ...
            $summary = trim(str_replace($prompt, '', $generatedText));

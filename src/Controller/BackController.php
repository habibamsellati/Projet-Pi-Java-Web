...
HTML;
    ...
    private function generateAISummary(string $text): string
    {
        ...
        $keywords = ['problème', 'défaut', 'erreur', 'demande', 'réclamation', 'insatisfait', 
                     ...
                     'produit', 'commande', 'livraison', 'service', 'qualité', 'délai'];
        ...
        foreach ($sentences as $index => $sentence) {
            ...
            // First sentence gets bonus (usually contains main issue)
            if ($index === 0) {
                $score += 3;
            }
        ...
        // Take top 2-3 sentences (max 40% of original)
        $maxSentences = max(2, min(3, (int)ceil(count($sentences) * 0.4)));
        $topSentences = array_slice($scoredSentences, 0, $maxSentences);
        ...
        // If summary is too similar to original (>90% length), return first 2 sentences
        if (strlen($summaryText) > strlen($text) * 0.9) {
            $firstTwo = array_slice($sentences, 0, 2);
            $summaryText = implode('. ', $firstTwo);

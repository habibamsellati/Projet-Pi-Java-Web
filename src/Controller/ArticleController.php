...
final class ArticleController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager, SluggerInterface $slugger, UserRepository $userRepository): Response
    {
        ...
        if ($user && $user->getRole() === User::ROLE_CLIENT) {
            $this->addFlash('error', 'Les clients ne peuvent pas cr̩er d\'articles.');
    ...
    public function show(Article $article, Request $request, EntityManagerInterface $entityManager, CommentaireReactionRepository $commentaireReactionRepository, ArticleRepository $articleRepository, BadWordFilterService $badWordFilter): Response
    {
        ...
        $commentReactionStats = $commentaireReactionRepository->getStatsForArticle((int) $article->getId());
        $userCommentReactions = [];
        if ($user instanceof User) {
            $userCommentReactions = $commentaireReactionRepository->getUserReactionsForArticle((int) $article->getId(), (int) $user->getId());
        }
    ...
    public function reactToComment(Commentaire $commentaire, string $type, Request $request, EntityManagerInterface $entityManager, CommentaireReactionRepository $commentaireReactionRepository): Response
    {
        $user = $this->getUser();
        $user = $user instanceof User ? $user : null;
        ...
        $reactionType = $type === 'like' ? CommentaireReaction::TYPE_LIKE : CommentaireReaction::TYPE_DISLIKE;
        $existingReaction = $commentaireReactionRepository->findOneBy([
            'commentaire' => $commentaire,
            'user' => $user,
        ]);

        if ($existingReaction instanceof CommentaireReaction) {

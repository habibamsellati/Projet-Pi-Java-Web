...
class LivraisonController extends AbstractController
{
    ...
    public function loadCalendarEvents(LivraisonRepository $livraisonRepository): JsonResponse
    {
        $user = $this->getUser();
        
        // Get livraisons based on user role
        if ($this->isGranted('ROLE_ADMIN')) {
            $livraisons = $livraisonRepository->findAll();
        } else {
            $livraisons = $livraisonRepository->findBy(['livreur' => $user]);
        }

        $events = [];
        foreach ($livraisons as $livraison) {
            if ($livraison->getDatelivraison()) {
                $color = match($livraison->getStatutlivraison()) {
                    'en_attente' => '#ffc107',
                    'en_cours' => '#17a2b8',
                    'livre' => '#28a745',
                    default => '#8b5e4a'
                ...
                $events[] = [
                    'id' => $livraison->getId(),
                    'title' => substr($livraison->getAddresslivraison(), 0, 30),
                    'start' => $livraison->getDatelivraison()->format('Y-m-d\TH:i:s'),
                    'backgroundColor' => $color,
                    'borderColor' => $color,
                    'url' => $this->generateUrl('app_livraison_show', ['id' => $livraison->getId()])
                ];
            }
        }
